...
import { AnkiDayStats } from "@/types";

interface AnkiWeeklyBarProps {
  data: AnkiDayStats[];
}
...
export default function AnkiWeeklyBar({ data }: AnkiWeeklyBarProps) {
    ...
    <div className="rounded-2xl border bg-[var(--color-card)] border-[var(--color-border)]" style={{ padding: "1.5rem" }}>
      ...
      <ResponsiveContainer width="100%" height={240}>
        <BarChart data={formatted} barCategoryGap="30%">
          ...
          <Bar dataKey="reviewed" name="Reviewed" fill="var(--color-primary)" radius={[4, 4, 0, 0]} />
          <Bar dataKey="newCards" name="New Cards" fill="var(--color-secondary)" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
